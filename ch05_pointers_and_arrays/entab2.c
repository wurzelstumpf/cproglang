 ...
 *    8 columns) spaces are printed using underscores (ie '_') and any columns
 *    traversed by a tab are printed using plus characters (ie '+').  So when
 *    testing don't use this characters in the input otherwise you will be
 *    confused (!:^*@).
 *
 */
#include <stdio.h>
    ...
    while ((c = getchar()) != EOF)
    {
        ...
        if (state == IN)
        {
            ...
            else      /* output an appropriate number of tabs and spaces */
            {
                ...
                if (pt < (nc - 1))
                    print_char('_', ((nc - 1) - pt)); /* output any remaining spaces */
            }
